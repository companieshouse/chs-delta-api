...
locals {
  ...
  service_name               = "chs-delta-api"
  container_port             = "5010"
  eric_port                  = "10000"
  docker_repo                = "chs-delta-api"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  lb_listener_rule_priority  = 55
  lb_listener_paths          = ["/delta*", "/chs-delta-api*"]
  healthcheck_path           = "/chs-delta-api/healthcheck" # healthcheck path for chs-delta-api 
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "chs-delta-api.env"
