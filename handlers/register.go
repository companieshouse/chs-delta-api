...
func Register(mainRouter *mux.Router, cfg *config.Config, kSvc services.KafkaService) error {
	...
	appRouter.HandleFunc("/delta/registers/delete", NewDeltaHandler(kSvc, h, chv, cfg, true, false, cfg.RegistersDeltaTopic).ServeHTTP).Methods(http.MethodPost).Name("registers-delta-delete")
